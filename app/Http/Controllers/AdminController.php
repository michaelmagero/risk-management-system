...
class AdminController extends Controller
{
    ...
    {
        return View('dev.users.show')->with('users', User::orderBy('created_at', 'desc')->get());
    }
    ...
    public function show($id)
    {
        ...
        if ($user) {
            return view('dev.users.read')->with('users', User::where('id', $user->id)->orderBy('created_at', 'desc')->paginate(10));
        } else {
        ...
        if ($validator->fails()) {
            ...
            if (Auth::user()->role == 'dev' || Auth::user()->role == 'admin') {
                ...
                if ($password != $confirm_password) {
                    Alert::error('Passwords Dont Match! Check passwords and try again', 'Error')->autoclose(3000);
                    return back();
                } else {
                    $user->password = bcrypt($request->get('password'));
                }
                $user->save();
                Alert::success('Update Successful!', 'Success')->autoclose(3000);
                return back();
            }
        }
    }
    ...
    public function show_itemcode($id)
    {
        ...
        if ($coupon) {
            return view('dev.itemcodes.read')
                ->with('coupons', Coupon::where('id', $coupon->id)->orderBy('created_at', 'desc')->paginate(10))
